...
pub(crate) fn deserialize_group<'de, G: Group + GroupEncoding, D: Deserializer<'de>>(d: D) -> Result<G, D::Error> {
    ...
    impl<'de, G: Group + GroupEncoding> Visitor<'de> for GroupVisitor<G> {
        ...
        fn visit_str<E>(self, v: &str) -> Result<Self::Value, E> where E: Error {
            ...
            hex::decode_to_slice(v, repr.as_mut()).map_err(|_| Error::invalid_value(Unexpected::Str(v), &self))?;
            bytes_to_group(&repr.as_ref()).ok_or(Error::invalid_value(Unexpected::Bytes(repr.as_ref()), &self))
        }
        ...
        fn visit_seq<A>(self, mut seq: A) -> Result<Self::Value, A::Error> where A: SeqAccess<'de> {
            ...
            bytes_to_group(repr.as_ref()).ok_or(Error::custom("unable to convert to a group element"))
        }
    }
