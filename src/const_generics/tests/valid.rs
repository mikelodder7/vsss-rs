...
use crate::{tests::utils::MockRng, util::bytes_to_field, Vec};
use elliptic_curve::{
    ff::PrimeField,
...
>() {
    ...
    let res = combine_shares::<F, S>(&shares);
    assert!(res.is_ok());
    let secret_1 = res.unwrap();
    assert_eq!(secret, secret_1);

    // Feldman test
    let res = feldman::split_secret::<F, G, _, 2, 3, S>(secret, None, &mut rng);
    assert!(res.is_ok());
    ...
    // Pedersen test
    let res = pedersen::split_secret::<F, G, _, 2, 3, S>(secret, None, None, None, &mut rng);
    assert!(res.is_ok());
    ...
    let bad_share = Share::<S>(Vec::from_slice(&[1u8; S]).unwrap());
...
>() {
    ...
    let res = shamir::split_secret::<F, _, THRESHOLD, LIMIT, S>(secret, &mut rng);
    assert!(res.is_ok());
    let shares = res.unwrap();

    let res = feldman::split_secret::<F, G, _, THRESHOLD, LIMIT, S>(secret, None, &mut rng);
    assert!(res.is_ok());
    let (feldman_shares, verifier) = res.unwrap();

    let res =
        pedersen::split_secret::<F, G, _, THRESHOLD, LIMIT, S>(secret, None, None, None, &mut rng);
    assert!(res.is_ok());
    ...
    for i in 0..5 {
        for j in 0..5 {
            ...
            for k in 0..5 {
                ...
                let res = combine_shares::<F, S>(parts);
                assert!(res.is_ok());
                let secret_1 = res.unwrap();
                assert_eq!(secret, secret_1);
