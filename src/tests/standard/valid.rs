...
>() {
    ...
    let res = shamir.split_secret::<F, _>(secret, &mut rng);
    assert!(res.is_ok());
    ...
    // Feldman test
    let res = Feldman { t: 2, n: 3 }.split_secret::<F, G, _>(secret, None, &mut rng);
    assert!(res.is_ok());
    ...
    // Pedersen test
    let res = Pedersen { t: 2, n: 3 }.split_secret::<F, G, _>(secret, None, None, None, &mut rng);
    assert!(res.is_ok());
...
pub fn combine_all<F: PrimeField + Zeroize, G: Group + GroupEncoding + Default + ScalarMul<F>>() {
    ...
    let res = Feldman { t: 3, n: 5 }.split_secret::<F, G, _>(secret, None, &mut rng);
    assert!(res.is_ok());
    let (feldman_shares, verifier) = res.unwrap();

    let res = Pedersen { t: 3, n: 5 }.split_secret::<F, G, _>(secret, None, None, None, &mut rng);
    assert!(res.is_ok());
