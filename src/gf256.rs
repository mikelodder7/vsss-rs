...
impl Field for Gf256 {
    ...
    fn try_random<R: TryRng + ?Sized>(rng: &mut R) -> Result<Self, R::Error> {
        ...
        rng.try_next_u32().map(|v| Self(v as u8))
    }
...
mod tests {
    ...
    fn combine_fuzz() {
        ...
        for _ in 0..25 {
            let threshold = rng.random::<u8>().saturating_add(1);

            let mut shares = Vec::with_capacity(threshold as usize);
            for i in 0..threshold {
                let share = vec![i; (rng.random::<u8>() as usize % 64) + 1];
                shares.push(share);
            }
    ...
    fn no_share_identifier_is_zero() {
        ...
        for _ in 0..100 {
            let secret = IdentifierGf256(Gf256(rng.random::<u8>()));
            let shares = shamir::split_secret::<GfShare>(3, 5, &secret, &mut rng).unwrap();
            for s in &shares {
                assert_ne!(
                    s.identifier.0.0, 0,
